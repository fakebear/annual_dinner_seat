...
<?php
	...
	$sql = "SELECT `reserve_table`,`Name` FROM `employee` WHERE `WWID` = $WWID";
		...
		if($result->num_rows >0) {
			...
			if($row['reserve_table']==0) {
				...
				if($row['reserve_table'] == $table) {
					...
					echo "<h2>".$row['Name']."</h2>";
					echo "<h2>You have reservation on table: ".$row['reserve_table'];
					//ask switch reservation
					$sql= "UPDATE `employee` SET `reserve_table`=$table WHERE `WWID`=$WWID";
					if($r=mysqli_query($con,$sql)) {
						printf("<h2>We now changed your reservation from table: %d to table: %d</h2>",$row['reserve_table'], $table);
					} else {
						echo "<h2>Something may goes wrong, please try again later.</h2>";
					}
				}
